...
;(function migrateQuestionsStudiedAt() {
  try {
    ...
    if (!cols.some((c) => c.name === 'studiedAt')) {
      db.exec('ALTER TABLE questions ADD COLUMN studiedAt TEXT')
    }
  } catch (e) {
    console.error('SQLite migrate studiedAt:', e.message)
  }
})()
...
app.post('/api/questions', authMiddleware, (req, res) => {
  ...
  db.prepare(`INSERT INTO questions (id,question,answer,detailedAnswer,codeExample,level,category,studied,correct,incorrect,answered,studiedAt,createdAt)
    VALUES (@id,@question,@answer,@detailedAnswer,@codeExample,@level,@category,0,0,0,0,NULL,@createdAt)`).run(q)
  db.prepare('UPDATE stats SET total = (SELECT COUNT(*) FROM questions) WHERE id = 1').run()
...
app.put('/api/questions/:id', authMiddleware, (req, res) => {
  ...
  db.prepare(`UPDATE questions SET question=@question,answer=@answer,detailedAnswer=@detailedAnswer,codeExample=@codeExample,level=@level,category=@category,studied=@studied,correct=@correct,incorrect=@incorrect,answered=@answered,studiedAt=@studiedAt WHERE id=@id`)
    .run({
      ...upd,
      studied: upd.studied ? 1 : 0,
      answered: upd.answered ? 1 : 0,
      studiedAt: upd.studiedAt != null && upd.studiedAt !== '' ? String(upd.studiedAt) : null,
    })
  db.prepare('UPDATE stats SET total = (SELECT COUNT(*) FROM questions) WHERE id = 1').run()
...
app.post('/api/questions/bulk', authMiddleware, (req, res) => {
  ...
  const insert = db.prepare(`INSERT OR IGNORE INTO questions (id,question,answer,detailedAnswer,codeExample,level,category,studied,correct,incorrect,answered,studiedAt,createdAt)
    VALUES (@id,@question,@answer,@detailedAnswer,@codeExample,@level,@category,0,0,0,0,NULL,@createdAt)`)
...
app.post('/api/questions/sync', (req, res) => {
  ...
  const upsert = db.prepare(`INSERT OR REPLACE INTO questions (id,question,answer,detailedAnswer,codeExample,level,category,studied,correct,incorrect,answered,studiedAt,createdAt)
    VALUES (@id,@question,@answer,@detailedAnswer,@codeExample,@level,@category,@studied,@correct,@incorrect,@answered,@studiedAt,@createdAt)`)
  db.transaction(arr =>
    arr.forEach(q => {
      if (!q || !q.id) return
      const existing = db.prepare('SELECT * FROM questions WHERE id = ?').get(q.id)
      const base = existing ? rowToQuestion(existing) : {}
      const merged = mergeQuestionUpdate(base, q)
      upsert.run(normalizeQuestionForUpsert(merged))
    })
  )(qs)
  db.prepare('UPDATE stats SET total = (SELECT COUNT(*) FROM questions) WHERE id = 1').run()
