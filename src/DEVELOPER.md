...
Вопросы хранятся в файле **`data.db`** (SQLite). На бесплатных веб-сервисах **файловая система контейнера сбрасывается при каждом redeploy**: после выкладки создаётся новая БД, срабатывает сид с **двумя демо-вопросами**, а данные, добавленные в админке на предыдущей версии, **теряются**.
